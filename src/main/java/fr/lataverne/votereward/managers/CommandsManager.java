...
public class CommandsManager {
    ...
    public void registerCommand(@NotNull CompositeCommand command) {
        ...
        try {
            ...
            if (this.commandMap.register("votereward", command)) {
                VoteReward.sendMessageToConsole(ChatColor.GREEN + "Registering the command \"" + command.getLabel() + "\" successful");
            } else {
                VoteReward.sendMessageToConsole(ChatColor.RED + "Failed to register command \"" + command.getLabel() + "\"");
            }
        } catch (NoSuchFieldException e) {
            Bukkit.getLogger().severe("Unable to register a command because the Bukkit server doesn't have commandMap");
        } catch (IllegalAccessException e) {
            Bukkit.getLogger().severe("The commandMapField is enforcing Java language access control and the underlying field is inaccessible");
        }
    }
    ...
    public void unregisterCommands() {
        ...
        try {
            ...
            Map<String, Command> knownCommands = (Map<String, Command>) this.commandMap.getClass().getMethod("getKnownCommands").invoke(this.commandMap);
            ...
            Bukkit.getLogger().severe("The known commands can't be retrieved because the object is enforcing Java language access control and the underlying method is inaccessible");
        }
    }
